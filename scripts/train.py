...
from paths import INPUT_PATH, OUTPUT_PATH, ROOT_PATH
...
def detect_buttons_and_create_annotations(employee_name):
    ...
    existing_classes_file = os.path.join(INPUT_PATH, "existingClasses.txt")#todo: this might has to be canged to the specific path
    ...
    model = YOLO(os.path.join(INPUT_PATH, model_name +".pt"))
...
def create_config_yaml(employee_name):
    ...
    config_file_path = os.path.join(INPUT_PATH, "config.yaml")
    class_file_path = os.path.join(INPUT_PATH, "existingClasses.txt")
...
def train_new_model(employee_name):
    ...
    epochs = 10 #the epochs the model will be trained for
    existing_classes_file = os.path.join(OUTPUT_PATH,employee_name, "existingClasses.txt")#todo: this might has to be canged to the specific path
    config_file = os.path.join(OUTPUT_PATH,employee_name,"config.yaml")
    ...
    results = model.train(data=config_file, epochs=epochs,name=employee_name, save=False)
